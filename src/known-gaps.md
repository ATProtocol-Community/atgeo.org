...
**Spatial AppViews** are the next logical step. We should build a Spatial AppView that identifies `place` objects in ATProtocol feeds, aggregates and indexes them, and them provides a `org.atgeo.searchRecords` query interface for searching them.
...
**User-generated places** are obviously necessary to 90% of ATProtcol apps. Enabling users to create places means providing moderation -- and not just content moderation, but also ground truthing. When you figure out how to solve this problem, please let us know!
